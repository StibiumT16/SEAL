...
# - $BART_FILES/bart.base/dict.txt - https://dl.fbaipublicfiles.com/fairseq/models/bart.base.tar.gz (decompress)
...
fairseq-preprocess --source-lang "source" --target-lang "target" \
    ...
    --destdir "$DATASET/bin" \
    --workers 60 \
    --srcdict $BART_FILES/bart.base/dict.txt \
    --tgtdict $BART_FILES/bart.base/dict.txt;

cp "${BART_FILES}/bart.base/dict.txt" "${DATASET}/dict.source.txt"
cp "${BART_FILES}/bart.base/dict.txt" "${DATASET}/dict.target.txt"
